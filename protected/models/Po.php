...
class Po extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('device_id, code, eff_date, created, users_id', 'required'),
			array('suppliers_id, device_id, online, users_id', 'numerical', 'integerOnly'=>true),
			array('code', 'length', 'max'=>45),
			array('remarks', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, suppliers_id, device_id, code, eff_date, remarks, created, online, users_id', 'safe', 'on'=>'search'),
		);
	}
