...
class TnController extends Controller {
    ...
    public function actionView($id) {
        ...
        $sql = "SELECT id FROM tn WHERE id = $id ORDER BY id DESC ";
    ...
    public function actionCreate() {
        try {
            ...
            $model->code = $this->returnCode("tn", "MTN-" . $model->device->code . "-");
    ...
    public function actionIndex() {
        ...
        $sql = "SELECT * FROM tn WHERE online >= 1 AND device_id IN ($device) $searchtxt ORDER BY id DESC ";
        $count = Yii::app()->db->createCommand($sql)->query()->rowCount;
        $dataProvider = new CSqlDataProvider($sql, array(
            'totalItemCount' => $count,
            'pagination' => array(
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'tn-form') {
