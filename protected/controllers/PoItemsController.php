...
class PoItemsController extends Controller {
    ...
    public function actionjsondata($id) {
        $data = PoItems::model()->findByPk($id);
    ...
    public function actionCreate() {
        try {
            ...
            //UPDATE ITEM COST
            $item = Items::model()->findByPk($model->items_id);
    ...
    public function actionupdateAll($id) {
        try {
            ...
            foreach ($_POST['qty'] as $key => $value) {
                ...
                $poitems = PoItems::model()->findByAttributes(array("id" => $key));
                $poitems->qty = $value;

                if (isset($_POST["notes"][$key])) {
                    $poitems->notes = $_POST["notes"][$key];
                }
                if (isset($_POST["cost"][$key])) {
                    $poitems->cost = $_POST["cost"][$key];
                }
                ...
                //UPDATE ITEM COST
                $item = Items::model()->findByPk($poitems->items_id);
                $item->cost = $poitems->cost;
                $item->save();
            }
    ...
    public function actionIndex() {
        ...
        $sql = "SELECT * FROM po_items WHERE online = 1 $searchtxt ORDER BY id DESC ";
    ...
    public function loadModel($id) {
        $model = PoItems::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'po-items-form') {
