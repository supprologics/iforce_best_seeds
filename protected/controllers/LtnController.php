...
class LtnController extends Controller {
    ...
    public function actionView($id) {
        ...
        $sql = "SELECT id FROM ltn WHERE id = $id ORDER BY id DESC ";
        ...
        if ($stsValue == 2) {
            $this->render('view_approval', array(
                'model' => $model,
                'dataProvider' => $dataProvider,
            ));
            return;
    ...
    public function actionCreate() {
        try {
            ...
            $model->code = $this->returnCode("ltn", "LT-" . $model->deviceFrom->code . "-");
    ...
    public function actionIndex() {
        ...
        $sql = "SELECT * FROM ltn WHERE online >= 1 $searchtxt ORDER BY id DESC ";
        $count = Yii::app()->db->createCommand($sql)->query()->rowCount;
        $dataProvider = new CSqlDataProvider($sql, array(
            'totalItemCount' => $count,
            'pagination' => array(
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'ltn-form') {
