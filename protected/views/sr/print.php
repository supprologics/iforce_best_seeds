...
$model = Sr::model()->findByPk($model->id);
$barcode = $model->code;
//HEADER CONTENT
ob_start();
?>
...
<div>
    ...
    <h3 style="text-align: center;">Supplier Return Note</h3>
            ...
            <tr>
                <td width="50%">
                    ...
                    <table cellspacing="0" cellpadding="0" width="100%" class="no-border">
                        ...
                        <tr>
                            <td>Doc Type</td>
                            <td><?php echo $this->returnSRTypes($model->sr_type); ?></td>
                        </tr>
...
<div>
    <table cellspacing="0" cellpadding="0" width="100%" class="borderd" style="overflow: wrap;">
        ...
        <?php
        $po_id = $model->id;
        $list = Yii::app()->db->createCommand("SELECT id FROM sr_items WHERE sr_id = '$po_id'")->queryAll();
        $num = 1;
        $tot = 0;
        $totamount = 0;
        ...
        foreach ($list as $value) {
            $poitems = SrItems::model()->findByPk($value['id']);
            ?>
            <tr>
                <td><?php echo $num; ?></td>
                ...
                <td style="text-align: right"><?php echo number_format($poitems->{$key} * $poitems->cost, 2); ?></td>
            </tr>
            <?php
            $num += 1;
            $tot += $poitems->{$key} * $poitems->cost;
        }
        ?>
