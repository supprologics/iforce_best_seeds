...
$model = Ltn::model()->findByPk($model->id);
$barcode = $model->code;
//HEADER CONTENT
ob_start();
?>
...
<div>
    ...
    <h3 style="text-align: center;">Internal Location Transfers</h3>
                ...
                <td width="50%">
                    ...
                    <table cellspacing="0" cellpadding="0" width="100%" class="no-border">
                        ...
                        <tr>
                            ...
                            <td><?php echo $model->deviceFrom->name; ?></td>
                        </tr>
                        <tr>
                            <td>Destination</td>
                            <td><?php echo $model->deviceTo->name; ?></td>
                        </tr>
...
<div>
    <table cellspacing="0" cellpadding="0" width="100%" class="borderd" style="overflow: wrap;">
        ...
        <?php
        $po_id = $model->id;
        $list = Yii::app()->db->createCommand("SELECT id FROM ltn_items WHERE ltn_id = '$po_id' AND qty > 0")->queryAll();
        $num = 1;
        $tot = 0;
        $totamount = 0;
        ...
        foreach ($list as $value) {
            $poitems = LtnItems::model()->findByPk($value['id']);
            ?>
            <tr>
                <td><?php echo $num; ?></td>
