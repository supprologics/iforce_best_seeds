<form action="#" method="post" id="inner_table">
    ...
    <table class="table table-sm table-hover table-sp" >
        ...
        <tbody id="inner_table_d" class="lineitems" >
            ...
            $po = Ltn::model()->findByPk($data['id']);
            $total = 0;
            $num = 1;

            $stock_lot = 1;
            $list = Yii::app()->db->createCommand("SELECT id FROM ltn_items WHERE ltn_id = '" . $data['id'] . "'")->queryAll();
            foreach ($list as $itemArray) {

                $item = LtnItems::model()->findByPk($itemArray['id']);
            ...
            <tr  data-id="<?php echo $item->id; ?>" >
                    ...
                    <td width="7%" class="" ><?php echo $item->batch_no; ?></td>
                    <td width="7%" class="" ><?php echo $item->expire_date; ?></td>
                    <td  width="7%" class="text-right"><?php echo $item->qty; ?></td>
