...
<script>
    ...
    $ (function () {
       ...
       var autocomplete = $ ("#items").tablecomplete ({
          minLength: 2,
          source: "<?php echo Yii::app()->createUrl('items/loadlistForLtn/' . $model->id); ?>",
       ...
       $ (document).on ("click", "#Ltn-print", function () {
          save ();
    ...
    function save () {
       ...
       $.ajax ({
          url: "<?php echo Yii::app()->createUrl("LtnItems/updateAll/" . $model->id) ?>",
          data: inner_data,
          type: "POST",
    ...
    $ (document).on ("click", "#Ltn-delete", function (e) {
       ...
       if (confirmdata == true) {
          $.ajax ({
             ...
             window.location.href = "<?php echo Yii::app()->createUrl('Ltn') ?>";
          });
       }
    });
    ...
    $ (document).on ("click", "#LtnItems-remove", function (e) {
       e.preventDefault ();
       ...
       do {
          ...
          $.ajax ({
             url: "<?php echo Yii::app()->createUrl('LtnItems/delete') ?>/" + val[i],
             success: showResponse,
             type: "POST",
             async: false,
             error: showResponse,
    ...
    $ (document).on ("click", "#Ltn-complete", function (e) {
       e.preventDefault ();
       ...
       if (confirmdata == true) {
          $.ajax ({
             url: "<?php echo Yii::app()->createUrl('Ltn/update') ?>/" + id,
             type: "POST",
             data: {
                online: 2
             },
          ...
          if (sts > 0) {
             window.location.href = "<?php echo Yii::app()->createUrl('Ltn') ?>";
             window.open ("<?php echo Yii::app()->createUrl('Ltn/print/') ?>/" + id, "mywindow", "location=1,status=1,scrollbars=1, width=800,height=600");
          }
       }else {
...
<div id="form_body">
    <div class="container-fluid">
        ...
        <h2>Internal Location Transfer Note - <?php echo $model->code; ?></h2>
        <div class="row" id="row_cont">
            <div class="col-sm-6">
                <div class="form-group row">
                    <label for="container_no" class="col-sm-3 control-label">Location From</label>
                    <div class="col-sm-6">
                        <?php echo $model->deviceFrom->code . " - " . $model->deviceFrom->name; ?>
                    </div>
                </div>
                <div class="form-group row">
                    <label for="container_no" class="col-sm-3 control-label">Location To</label>
                    <div class="col-sm-6">
                        <?php echo $model->deviceTo->code . " - " . $model->deviceTo->name; ?>
                    </div>
                </div>
        ...
        <div id="inline_manu">
            <div class="row">
                <div class="col-sm-10">
                    <form action="<?php echo Yii::app()->createUrl('LtnItems/create') ?>" method="post" id="LtnItems-form" >
                        <input type="hidden" name="ltn_id" id="ltn_id" value="<?php echo $model->id; ?>" />
                        <div class="form-row">
                            <div class="col-3">
                            ...
                            <div class="col-1" id="con_d">
                                <label>EXPIRE</label>
                                <input type="text"  data-date-container="#con_d" id="expire_date" name="expire_date" class="form-control form-control-sm" placeholder="Expire">
                            </div>
                            <div class="col-1">
                                <label>MRP</label>
                                <input type="text" readonly="true"  min="0" id="selling" name="selling" class="form-control form-control-sm" placeholder="MRP Rs.">
                            </div>
                            <div class="col-1">
                                <label>Qty</label>
        ...
        <div id="quick_nav">
            ...
            <button id="LtnItems-remove" class="btn btn-block btn-sm btn-danger"><span class="fas fa-times-circle text-danger"></span></button>
            <button id="Ltn-print" data-id="<?php echo $model->id; ?>" class="btn btn-block btn-sm btn-primary"><span class="oi oi-print"></span></button>
        </div>
        ...
        <div id="btn_bar" class="mt-2 text-right">
            <div class="row">
                <div class="col">
                    <button id="Ltn-delete" data-id="<?php echo $model->id; ?>" class="btn btn-sm btn-danger"> Revoke <span class="fas fa-times-circle text-danger"></span></button>
                    <button id="Ltn-complete" data-id="<?php echo $model->id; ?>" class="btn  btn-sm btn-success">Complete</button>
                </div>
            </div>
        </div>
