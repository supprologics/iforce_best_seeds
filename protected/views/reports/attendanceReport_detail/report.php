...
$device = Device::model()->findByPk($_POST['devices_id']); 
            ...
            <div class="row" style="margin-bottom: 8px;">
                <div class="col">
                    ...
                    <table width="100%" class="table">
                        <tr>
                            <td style="font-weight: bold;" width="10%">Date Range</td>
                            <td><?php echo $_POST['date_from']; ?> - <?php echo $_POST['date_to']; ?></td>
                        </tr>
                        <tr>
                            <td style="font-weight: bold;">Device</td>
                            <td><?php echo $device->code." - ". $device->name; ?></td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div id="popularity_div">
                <table id="popularity" class="table data reports_table table-sm table-bordered" cellspacing="0" cellpadding="0" width="100%">
                    ...
                    foreach ($daterange as $date) {
                        ...
                        $loginIN = Yii::app()->db->createCommand("SELECT id FROM session_logins WHERE device_id = '$device_id' AND DATE(created) = '$eff_date' AND log_type = 1 ORDER BY created ASC LIMIT 1")->queryRow();
                        $loginOUT = Yii::app()->db->createCommand("SELECT id FROM session_logins WHERE device_id = '$device_id' AND DATE(created) = '$eff_date' AND log_type = 2 ORDER BY created DESC LIMIT 1")->queryRow();
                        ...
                        <tr>
                            <td><?php echo $num; ?></td>
                            <td><?php echo $eff_date; ?></td>
                            <td><?php echo!empty($sessionIn) ? $sessionIn->areas->name : ""; ?></td>
                            <td><?php echo!empty($sessionIn) ? $this->distance($device->lat, $device->lng, $sessionIn->latitude, $sessionIn->longitude, "K") : ""; ?></td>
                            ...
                            <td class="text-center"><?php echo!empty($sessionOut) ? "<a href='https://maps.google.com/?q=" . $sessionOut->latitude . "," . $sessionOut->longitude . "' target='_blank'>MAP</a>" : ""; ?></td>
                             <td><?php echo!empty($sessionIn) ? $this->distance($device->lat, $device->lng, $sessionOut->latitude, $sessionOut->longitude, "K") : ""; ?></td>
