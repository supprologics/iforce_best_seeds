    ...
    <body>
        ...
        <div class="report_body" >
            ...
            <table id="popularity" class="table data reports_table table-sm table-bordered" cellspacing="0" cellpadding="0" width="100%">
                ...
                $num = 1;
                $list = Yii::app()->db->createCommand("SELECT id FROM sr WHERE device_id = '$devices_id' AND eff_date >= '$datefrom' AND eff_date <= '$dateto' ")->queryAll();
                $tot = 0;
                foreach ($list as $value) {
                    $att = Sr::model()->findByPk($value['id']);
                    $totpo = Yii::app()->db->createCommand("SELECT SUM((qty + qty_ns) * cost) as tot FROM `sr_items` where sr_id = '". $value['id'] ."'")->queryAll();
