...
$model = Po::model()->findByPk($model->id);
$barcode = $model->code;
//HEADER CONTENT
ob_start();
?>
...
<div>
    ...
    <h3 style="text-align: center;">Purchasing Order</h3>
...
<div>
    <table cellspacing="0" cellpadding="0" width="100%" class="borderd" style="overflow: wrap;">
        ...
        <?php
        $po_id = $model->id;
        $list = Yii::app()->db->createCommand("SELECT id FROM po_items WHERE po_id = '$po_id'")->queryAll();
        $num = 1;
        $tot = 0;
        $totamount = 0;
        ...
        foreach ($list as $value) {
            $poitems = PoItems::model()->findByPk($value['id']);
            ?>
            <tr>
                <td><?php echo $num; ?></td>
                ...
                <td style="text-align: right"><?php echo number_format($poitems->qty * $poitems->cost, 2); ?></td>
            </tr>
            <?php
            $num += 1;
            $tot += $poitems->qty * $poitems->cost;
        }
        ?>
